...
// Linear distance annotation between two points in a 2D plane.
//
...
//
// `plane` records which drawing plane the (start, end) coords refer to:
//   0 = Top (XY), 1 = Front (XZ), 2 = Right (YZ). Renderers show dimensions
//   only in the view whose plane matches, keeping annotations aligned with
//   the surface they were authored on.
struct Dimension {
    EntityId id{};
    EntityId group_id{};
    EntityId layer_id{};
