#pragma once

#include <optional>

#include <QPointF>
#include <QString>

#include "Tool.hpp"

namespace cadino::ui {

// Two-click leader/callout placement: first click anchors the arrow,
// second click drops the text. The string is supplied at activation
// via dialog.
class LeaderTool : public Tool {
public:
    explicit LeaderTool(QString text, double height = 120.0)
        : text_{std::move(text)}, height_{height} {}

    void on_press(PlanView& view, QPointF model_pos, Qt::MouseButton button) override;
    void on_move(PlanView& view, QPointF model_pos) override;
    void on_release(PlanView& view, QPointF model_pos, Qt::MouseButton button) override;
    void paint_overlay(QPainter& p, const PlanView& view) const override;
    void on_cancel(PlanView& view) override;

private:
    QString text_;
    double height_{120.0};
    std::optional<QPointF> anchor_;
    QPointF hover_;
};

}  // namespace cadino::ui
